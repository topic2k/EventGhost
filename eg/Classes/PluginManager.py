...
REPOSITORY_URL = "http://torsti.net/repo/plugins.php"
...
class Repository(object):
    ...
    def DoFetching(self):
        ...
        if eg.pluginManager.fetched_in_session:
            return
        try:
    ...
    def ParseDownloadedXml(self):
        ...
        try:
            ...
            eg.PrintError(
                "{0}\n({1})".format(
                    Text.ErrorParsingRepoData, err.message
                )
            )
            return
        ...
        for plgn in available_plugins:
            ...
            try:
                ...
                plugin_info.date_added = plgn.get("date_added", None)
